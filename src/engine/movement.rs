...
pub fn process_entity_movement(
    ...
    let movement_data = if let Some(entity) = entities.get(entity_id) {
         match &entity.entity_type {
            crate::state::entities::EntityType::Creature(c) => Some((c.current_path.clone(), c.move_timer, c.movement_speed)),
            crate::state::entities::EntityType::Hero(h) => Some((h.current_path.clone(), h.move_timer, h.movement_speed)),
            _ => None,
        }
    } else {
        None
                ...
                match &mut entity.entity_type {
                    crate::state::entities::EntityType::Creature(c) => {
                         ...
                         if let Some(ref mut path) = c.current_path {
                             if !path.is_empty() { path.remove(0); }
                             if path.is_empty() { c.current_path = None; }
                         }
                    }
                    crate::state::entities::EntityType::Hero(h) => {
                         ...
                         if let Some(ref mut path) = h.current_path {
                             if !path.is_empty() { path.remove(0); }
                             if path.is_empty() { h.current_path = None; }
                         }
                    }
                    _ => {}
                }
