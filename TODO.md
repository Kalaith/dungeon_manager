...
- Hero-building destruction effects are live and now **explain themselves** — razing the armoury reads "Armory destroyed — every hero loses 2 attack and 2 defence" rather than just naming the building. Remaining follow-ups:
  ...
  - Verified by test, not by observation. Nothing in a capture attacks the hero base, so a destruction has never been *seen*. A `raid` scene setting `attack_marker` at the hero base would fix that, but the capture starts with 2 minions against 200 HP buildings, so it needs a seeded army to resolve in a reasonable frame count.
