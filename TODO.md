...
- `tests/live_data_fields_tests.rs` guards `EffectsData`, `TrapEffects`, `TileData` and `MonsterData` against fields that are parsed and read nowhere. Five documented exceptions remain, each a real gap rather than an oversight: `hero_conversion_rate` (prison conversion unbuilt), `lockable` (magical door locking — see Engine above), `trigger_type` (every trap is `"pressure"`), `durability` (dig cost comes from config; per-tile durability is a balance change) and `damage_per_second` (lava authors 25 but also blocks movement, so nothing can stand in it — the environmental-hazards item).
- The guard only checks *top-level* struct fields. `MonsterData` passes trivially because its fields are all sub-structs (`stats`, `combat`, `economy`, …) whose own fields have never been audited — that is where the next dead data is most likely hiding.
