...
- `tests/live_data_fields_tests.rs` sweeps **every** `pub struct` under `src/data/` for fields that are parsed and read nowhere, so a new struct cannot arrive with dead fields unnoticed. The first full sweep found **48**, now listed in `UNCONSUMED` with reasons. That list is the honest inventory of authored-but-inert content data; the biggest clusters are worth their own items:
  - **Room build rules** — `dig_required`, `requires_claimed`, `can_overlap`, `allowed_terrain`, `construction_time` are all ignored. Rooms can be placed on any player tile regardless of the terrain the data allows, and appear instantly rather than taking `construction_time` to build.
  - **Room limits** — `max_instances` and `forbidden_if` unenforced, so "one per dungeon" rooms are not.
  - **The whole hero behaviour model** — `trap_awareness`, `door_break_chance`, `light_preference`, `fear_resistance`, `will_fight_to_death`, `call_for_aid`, `bravery`. Every hero is authored with a personality that hero AI never consults.
  - **Hero building destruction effects** — authored on all 12 buildings, applied by nothing, so destroying the town hall does not win and destroying the armory does not weaken heroes.
  - **Spell `souls` cost and targeting restrictions** (`requires_visibility`, `valid_targets`).
  - `ResourceData::mine_value` — this *is* the "gem seams pay a flat 25" item above: the 25 is hardcoded, and the data field beside it has never been read.
