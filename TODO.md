...
- `tests/live_data_fields_tests.rs` now fails if an `EffectsData` field is parsed and read nowhere; `hero_conversion_rate` is its one documented exception, waiting on prison hero-to-creature conversion. The same guard would be worth having for `TrapEffects`, `TileData` and `MonsterData`, which have never been audited.
...
- Wire the last of the finished-but-unreachable art: three hero buildings (`blacksmith`, `guard_tower`, `tavern`) need spawn/destruction rules in `hero_buildings.json`. The allowlist in `tests/asset_manifest_tests.rs` names them. (The fire, gas and lightning traps are done.)
