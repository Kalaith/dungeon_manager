...
- **A bug class the field guard cannot see**: `global_rooms_required` was read — but only to draw a tooltip, so the build button promised "Requires: Library" while the engine let you build without one. Fields that are *displayed* but not *enforced* look alive to any reference-counting check. Worth auditing the other requirement-ish fields the UI renders (`requirements.research` is shown the same way; verify the unlock path actually gates it).
