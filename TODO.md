...
- Author the remaining `docs/monsters.md` roster. Three tiers are done — undead (**Lich**, **Grave Hulk**, on `necromancy`), demons (**Balor**, **Infernal Hound**) and casters (**Cultist** on `ritual_tech`, **Hexbinder** on `war_magic`). Still to author: Stone Warden, Gnoll, Kobold, Ogre, Ironbound, Shadow Stalker, Assassin Wisp, Beastling, Flesh Amalgam, Void-Touched, Overseer, Archivist.
...
- Lighting and atmosphere pass; the GDD's "warped evil" theming. The *sprite baking* light is now correct (see the key-light item under Content), so this is about in-game lighting — torch pools, fog, coloured room glow — rather than about how the 64px art is shaded.
