...
- The screenshot harness's `gameplay` scene never gets past the mission intro modal, so 150 frames of it exercise no combat, digging or spawning. Every visual verification this repo has done is of that one screen. A scene that dismisses the modal and runs the simulation would make the harness able to catch far more.
